...
#| fig-cap: One simulated 40-year wealth path
...
for _ in range(N_years):
    balance *= mult_heads if np.random.random() < p_heads else mult_tails
    path.append(balance)

final_balance_single = path[-1]

fig, ax = plt.subplots(figsize=(8, 4))
ax.plot(range(len(path)), path, color="steelblue", linewidth=1.6)
ax.axhline(y=buy_in, color="gray", linestyle="--", alpha=0.8, label=f"Buy-in ${buy_in:,}")
ax.set_xlabel("Year")
ax.set_ylabel("Account balance ($)")
ax.set_title("Single simulation trajectory")
ax.legend()
...
```{python}
#| label: fig-dist-100
#| fig-cap: Distribution of final balances over 100 simulations (log x-scale)
np.random.seed(123)
n_sims = 100
finals_original = []
...
ax.hist(finals_original, bins=25, color="steelblue", edgecolor="white", alpha=0.9, log=True)
ax.axvline(x=buy_in, color="coral", linestyle="--", linewidth=2, label=f"Buy-in ${buy_in:,}")
ax.axvline(x=q50, color="black", linestyle="-", linewidth=1.4, label=f"Median ${q50:,.0f}")
ax.axvspan(q10, q90, color="gold", alpha=0.2, label="Middle 80%")
ax.set_xscale("log")
ax.set_xlabel("Final balance ($, log scale)")
ax.set_ylabel("Count (log scale)")
ax.set_title("Highly right-skewed outcomes")
ax.text(
    0.02,
    0.98,
    f"Mean: ${mean_final:,.0f}\nMedian: ${median_final:,.0f}\nP(Final > $30k): {prob_above_30k:.1%}",
...
```{python}
#| label: fig-25pct-compare
#| fig-cap: Original strategy vs 25% risk strategy (final wealth distributions)
...
ax.set_title("25% strategy improves reliability")
ax.text(
    0.02,
    0.98,
...
#| fig-cap: "Distribution of final balances across 100 simulations (log scale with summary annotations)"
np.random.seed(123)
n_sims = 100
finals_original = []
...
fig, ax = plt.subplots(figsize=(9, 4.8))
ax.hist(finals_original, bins=25, color="steelblue", edgecolor="white", alpha=0.85, log=True)
ax.axvline(x=buy_in, color="coral", linestyle="--", linewidth=2, label=f"Buy-in ${buy_in:,}")
ax.axvline(x=q50, color="black", linestyle="-", linewidth=1.5, label=f"Median ${q50:,.0f}")
ax.axvspan(q10, q90, color="gold", alpha=0.2, label="Middle 80% range")
ax.set_xscale("log")
ax.set_xlabel("Final balance ($, log scale)")
ax.set_ylabel("Count (log scale)")
ax.set_title("Final balances are highly right-skewed (100 simulations)")
ax.text(
    0.02, 0.98,
    f"Mean: ${mean_final:,.0f}\nMedian: ${median_final:,.0f}\nP(Final > $30k): {prob_above_30k:.1%}",
    transform=ax.transAxes,
